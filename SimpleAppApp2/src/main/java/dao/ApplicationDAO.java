...
public class ApplicationDAO extends DAO {
	...
	public EmployeeBean select(String empId) {
		Connection con = dbConnect();
		PreparedStatement st = null;
		ResultSet rs = null;
		EmployeeBean dto = null;

		String sql = "SELECT e.emp_name, d.dpt_name FROM employees e "
				+ "JOIN departments d ON e.dpt_id = d.dpt_id "
				+ "WHERE e.emp_id = ? AND e.is_deleted = 0";
		...
		try {
			if (con != null) {
				...
				if (rs.next()) {
					...
					dto.setDptName(rs.getString("dpt_name"));
				}
			}
		} catch (SQLException e) {
			System.out.println("selectエラー");
			System.out.println(e.getMessage());
		}
		dbClose(con);
		return dto;

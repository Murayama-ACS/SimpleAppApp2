...
<body>
	<h2>
		申請一覧画面（ステータスID:
		<%= currentStatus != null ? currentStatus : "1" %>）
	</h2>
	<p>
		ログインユーザー:
		<%= empName %>
	...
	<form action="<%= request.getContextPath() %>/ApplicationWaitList"
		method="get">
		<label>表示ステータスID: </label> <input type="text" name="pendingStatus"
			value="<%= currentStatus != null ? currentStatus : "1" %>" size="5">
			...
			<tr>
				...
				<td>
					<div style="display: flex; gap: 5px;">
						...
						<button type="button"
							onclick="openApprovalModal('<%= app.getApctId() %>', <%= app.getStatus_id() %>)">承認</button>

						<button type="button"
							onclick="openRejectModal('<%= app.getApctId() %>')">却下</button>
					</div>
				</td>
			</tr>
			<% } %>
	...
	<div id="actionModal" class="modal">
		<div class="modal-content">
			...
			<form action="<%= request.getContextPath() %>/ApplicationWaitList"
				method="post">
				<input type="hidden" id="modalApctId" name="apct_id"> <input
					type="hidden" id="modalNextStatus" name="next_status_id"> <input
					type="hidden" name="pendingStatus"
					value="<%= currentStatus != null ? currentStatus : "1" %>">

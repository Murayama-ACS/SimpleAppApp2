...
<body>
	...
	if ("true".equals(request.getParameter("login"))) {
		...
		out.println("<p style='color:red;'>エラー: ID「" + testEmpId
				+ "」の社員データがDBに見つかりません。先にemployeesテーブルへデータを登録するか、SQLが反映されているか確認してください。</p>");
			}
		} else {
	...
	<form action="login_mock.jsp" method="get">
		<input type="hidden" name="login" value="true"> <label
			for="emp_select">ログインユーザー選択: </label> <select name="emp_id"
			id="emp_select" style="padding: 5px; width: 450px;">
			...
			value="模擬ログインして申請ステータス変更一覧画面へ" style="padding: 5px 15px;">
	</form>
</body>
</html>
...
<body>
	...
	<div class="header-info">
		ログイン社員:
		<%=(employee != null) ? employee.getEmp_name() : ""%>
		(部署:
		<%=dptName != null ? dptName : "未設定"%>
		/ 役職:
		<%
	if ("E04".equals(posId))
		out.print("社長");
	else if ("E03".equals(posId))
		out.print("本部長");
	else if ("E02".equals(posId))
		out.print("部長");
	...
	<div class="search-box">
		<form
			action="${pageContext.request.contextPath}/ApplicationHistoryServlet"
			method="get">
			<input type="hidden" name="scope"
				value="<c:out value='${currentScope}'/>" /> <input type="hidden"
				name="filter"
				value="<c:out value='${currentStatusFilter == \"incomplete\" ? \"unapproved\" : \"all\"}'/>" />
			...
			<c:if test="${empBean.pos_id eq 'E03' || empBean.pos_id eq 'E04'}">
                ...
                <select name="q_department">
					...
					<c:forEach var="d" items="${dptList}">
						<option value="${d.dpt_id}"
							<c:if test="${q_department eq d.dpt_id}">selected</c:if>>
			...
			&nbsp;&nbsp;種別: <select name="q_type">
				<option value="">--指定なし--</option>
				<option value="経費精算"
					<c:if test="${q_type eq '経費精算'}">selected</c:if>>経費精算</option>
				<option value="旅費精算"
					<c:if test="${q_type eq '旅費精算'}">selected</c:if>>旅費精算</option>
			</select>

			<!-- 【修正】金額を「円以上〜円以下」の範囲指定入力に拡張 -->
			&nbsp;&nbsp;金額範囲: <input type="number" name="q_amount_min"
			...
			<input type="hidden" name="dir" value="<c:out value='${dir}'/>" /> <input
	...
	<div class="tab-group">
		<a
			href="ApplicationHistoryServlet?scope=self&filter=<%="incomplete".equals(currentStatusFilter) ? "unapproved" : "all"%>"
			class="tab <%="self".equals(currentScope) ? "active" : ""%>">自身</a>
		<%
		if (!"E00".equals(posId)) {
		%><a
			href="ApplicationHistoryServlet?scope=subordinate&filter=<%="incomplete".equals(currentStatusFilter) ? "unapproved" : "all"%>"
		...
		%><a
			href="ApplicationHistoryServlet?scope=management&filter=<%="incomplete".equals(currentStatusFilter) ? "unapproved" : "all"%>"
	...
	<div class="tab-group">
		<a
			href="ApplicationHistoryServlet?scope=<%=currentScope%>&filter=unapproved"
			class="tab <%="incomplete".equals(currentStatusFilter) ? "active" : ""%>">未完了</a>
		<a
			href="ApplicationHistoryServlet?scope=<%=currentScope%>&filter=all"
		...
		<thead>
			<tr>
				...
				<c:url var="sortName" value="/ApplicationHistoryServlet">
					...
					<c:param name="dir"
						value="${sort == 'name' && dir == 'asc' ? 'desc' : 'asc'}" />
					<c:param name="page" value="1" />
					<c:param name="q_status" value="${q_status}" />
					<c:param name="q_name" value="${q_name}" />
					<c:param name="q_department" value="${q_department}" />
				...
				<c:url var="sortDpt" value="/ApplicationHistoryServlet">
					...
					<c:param name="dir"
						value="${sort == 'dpt' && dir == 'asc' ? 'desc' : 'asc'}" />
					<c:param name="page" value="1" />
					<c:param name="q_status" value="${q_status}" />
					<c:param name="q_name" value="${q_name}" />
					<c:param name="q_department" value="${q_department}" />
				...
				<th><a href="${sortDpt}">部門<c:if test="${sort == 'dpt'}">
							<c:out value="${dir == 'asc' ? ' ▲' : ' ▼'}" />
						</c:if></a></th>
				...
				<c:url var="sortType" value="/ApplicationHistoryServlet">
					...
					<c:param name="dir"
						value="${sort == 'type' && dir == 'asc' ? 'desc' : 'asc'}" />
					<c:param name="page" value="1" />
					<c:param name="q_status" value="${q_status}" />
					<c:param name="q_name" value="${q_name}" />
					<c:param name="q_department" value="${q_department}" />
				...
				<c:url var="sortMethod" value="/ApplicationHistoryServlet">
					...
					<c:param name="filter"
						value="${currentStatusFilter == 'incomplete' ? 'unapproved' : 'all'}" />
					<c:param name="sort" value="method" />
					<c:param name="dir"
						value="${sort == 'method' && dir == 'asc' ? 'desc' : 'asc'}" />
					<c:param name="page" value="1" />
					<c:param name="q_status" value="${q_status}" />
					<c:param name="q_name" value="${q_name}" />
					<c:param name="q_department" value="${q_department}" />
				...
				<th><a href="${sortMethod}">支払方法<c:if
							test="${sort == 'method'}">
							<c:out value="${dir == 'asc' ? ' ▲' : ' ▼'}" />
						</c:if></a></th>
				...
				<c:url var="sortAmount" value="/ApplicationHistoryServlet">
					...
					<c:param name="dir"
						value="${sort == 'amount' && dir == 'asc' ? 'desc' : 'asc'}" />
					<c:param name="page" value="1" />
					<c:param name="q_status" value="${q_status}" />
					<c:param name="q_name" value="${q_name}" />
					<c:param name="q_department" value="${q_department}" />
				...
				<c:url var="sortStatus" value="/ApplicationHistoryServlet">
					...
					<c:param name="dir"
						value="${sort == 'status' && dir == 'asc' ? 'desc' : 'asc'}" />
					<c:param name="page" value="1" />
					<c:param name="q_status" value="${q_status}" />
					<c:param name="q_name" value="${q_name}" />
					<c:param name="q_department" value="${q_department}" />
				...
				<th><a href="${sortStatus}">申請状況<c:if
							test="${sort == 'status'}">
							<c:out value="${dir == 'asc' ? ' ▲' : ' ▼'}" />
						</c:if></a></th>
				...
				<c:url var="sortDate" value="/ApplicationHistoryServlet">
					...
					<c:param name="dir"
						value="${sort == 'date' && dir == 'asc' ? 'desc' : 'asc'}" />
					<c:param name="page" value="1" />
					<c:param name="q_status" value="${q_status}" />
					<c:param name="q_name" value="${q_name}" />
					<c:param name="q_department" value="${q_department}" />
				...
				<th><a href="${sortDate}">申請日<c:if test="${sort == 'date'}">
							<c:out value="${dir == 'asc' ? ' ▲' : ' ▼'}" />
						</c:if></a></th>
		...
		<tbody>
			...
			if (historyList != null && !historyList.isEmpty()) {
				for (ApplicationBean app : historyList) {
					...
					// 【変更】管理部削除特権：管理スコープでの全社表示時、ステータスが4以下であれば削除可能
			...
			<tr>
				...
				<td><%=app.getCreateDate() != null
		? app.getCreateDate().format(java.time.format.DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm"))
		: "---"%>
				</td>
				<td>
 ...
 if (canEditOrDelete) {
 %>
					<form action="ApplicationEdit" method="post"
						style="display: inline;">
						<input type="hidden" name="apct_id" value="<%=app.getApctId()%>">
 ...
 %>
			...
			%>
			<tr>
				<td colspan="10" style="text-align: center;">該当する申請履歴がありません。</td>
			</tr>
			<%
			}
			%>
	...
	%>
	<script type="text/javascript">alert("<%=errorMsg%>
		");
	</script>
	<%
	}

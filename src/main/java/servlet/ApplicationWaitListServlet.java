...
public class ApplicationWaitListServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) 
        ...
        if (sortKey == null || sortKey.isEmpty()) sortKey = "urgent"; // デフォルトは緊急度
        
        String sortDir = request.getParameter("dir");
        if (sortDir == null || sortDir.isEmpty()) sortDir = "DESC"; // デフォルトは降順（緊急を上に）
        ...
        try {
            ...
            if (queryString != null && !queryString.isEmpty()) {
                // ポップアップ用のワンタイムパラメータを履歴URLから除外する（正規表現で削除）
